...
from torch_geometric.nn import GCNConv, JumpingKnowledge, TopKPooling, SAGPooling
...
def dense_adj_to_edge_index(adj_dense):
    edge_index = (adj_dense > 0).nonzero(as_tuple=False).t().contiguous()
...
class PGP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_clusters,device,merge_mode, pooling_method='diffpool', pool_ratio=0.5):
        ...
        self.topk_pool = TopKPooling(hidden_dim, ratio=pool_ratio)
        self.sag_pool = SAGPooling(hidden_dim, ratio=pool_ratio)
        self.readout_fc = nn.Linear(hidden_dim, output_dim)

    def _readout(self, x, mode):
        if mode == 'mean':
    ...
    def forward(self, x, adj_dense):
        ...
        if self.pooling_method == 'diffpool':
            ...
            out, graph_emb = self.DiffPool_network(x_ass,adj_ass)
            return x_ass, cluster_assignments, entropy_mean, out, graph_emb

        if self.pooling_method in {'mean', 'max', 'sum'}:
            graph_emb = self._readout(x_encoder, self.pooling_method)
            out = self.readout_fc(graph_emb)
            return x_encoder, cluster_assignments, entropy_mean, out, graph_emb

        if self.pooling_method in {'topk', 'sag'}:
            edge_index = dense_adj_to_edge_index(adj_dense.float()).to(x_encoder.device)
            batch = torch.zeros(x_encoder.size(0), dtype=torch.long, device=x_encoder.device)
            if self.pooling_method == 'topk':
                pooled_x, _, _, _, _, _ = self.topk_pool(x_encoder, edge_index, batch=batch)
            else:
                pooled_x, _, _, _, _, _ = self.sag_pool(x_encoder, edge_index, batch=batch)
            graph_emb = torch.mean(pooled_x, dim=0)
            out = self.readout_fc(graph_emb)
            return pooled_x, cluster_assignments, entropy_mean, out, graph_emb

        raise ValueError(f"Unsupported pooling_method: {self.pooling_method}")
